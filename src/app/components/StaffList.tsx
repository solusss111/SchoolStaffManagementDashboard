...
export function StaffList() {
  ...
  const filteredData = data.filter((staff) => {
    ...
    const matchesSearch = !query ||
      ...
      staff.subject.toLowerCase().includes(query) ||
    ...
    if (categoryFilter !== 'all') {
      ...
      if (categoryFilter === 'high') matchesCategory = cat.includes('высшая');
      else if (categoryFilter === 'first') matchesCategory = cat.includes('первая');
      else if (categoryFilter === 'second') matchesCategory = cat.includes('вторая');
      else if (categoryFilter === 'none') matchesCategory = cat.includes('без');
    }

    // Предмет
    let matchesSubject = true;
    if (subjectFilter !== 'all') {
      const sub = staff.subject.toLowerCase();
      if (subjectFilter === 'math') matchesSubject = sub.includes('математика');
      else if (subjectFilter === 'physics') matchesSubject = sub.includes('физика');
      else if (subjectFilter === 'chemistry') matchesSubject = sub.includes('химия');
      else if (subjectFilter === 'biology') matchesSubject = sub.includes('биология');
      else if (subjectFilter === 'history') matchesSubject = sub.includes('история');
      else if (subjectFilter === 'kazakh') matchesSubject = sub.includes('казах');
      else if (subjectFilter === 'english') matchesSubject = sub.includes('английск');
      else if (subjectFilter === 'primary') matchesSubject = sub.includes('начальн');
    }

    // Год
    let matchesYear = true;
    if (yearFilter !== 'all') {
      matchesYear = staff.nextAttestation.includes(yearFilter);
    }

    return matchesSearch && matchesCategory && matchesSubject && matchesYear;
  });
